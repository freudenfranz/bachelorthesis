...
omnet_federate_url="http://www.dcaiti.tu-berlin.de/research/simulation/download/get/omnetpp-patch-18.0.zip"
...
print_usage() {
   ...
   log "\n -p, --regen-protobuf\ŧ\tRegenerate Protobuf c++ source, when using a different version of protobuf 3."
   log "\n -q, --quiet"
...
build_omnet_federate()
{
   ...
   if [ "$arg_regen_protobuf" = "true" ]; then
      protoc --cpp_out="$omnet_federate_installation_path/src/util/" --proto_path=$omnet_federate_installation_path $omnet_federate_installation_path/ClientServerChannelMessages.proto
   fi
